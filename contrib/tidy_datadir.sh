#!/bin/bash
# Copyright (c) 2013 The Jobecoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Jobecoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Jobecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Jobecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Jobecoin 0.7 datadir."
    ;;
  3)
    echo "Detected Jobecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Jobecoin 0.8 datadir."
    ;;
esac
