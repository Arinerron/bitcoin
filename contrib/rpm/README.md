...
The RPM spec file provided here is for Jobecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://jobecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Jobecoin git repository](https://github.com/jobecoin/jobecoin) but are not part of
...
Currently only `Source10` is used, for the example `jobecoin.conf` file.
...
in the jobecoin git repository. At present only an SVG version of the Jobecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the jobecoin
...
needed when 0.12.1 is released, a proper fix is already in the Jobecoin git
master branch.
...
idea to build Jobecoin Core against a static version of that library compiled
...
The `jobecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `jobecoin-server` package. At this point, most RPM based
...
The `jobecoin-server` package should have SELinux support. How to properly do
...
Jobecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/jobecoin/). He has been
packaging Jobecoin for Fedora at least since 2012.
...
4. I renamed the `jobecoin` package that contains the Qt GUI to `jobecoin-core` as
that appears to be how the general population refers to it, in contrast to
`jobecoin-xt` or `jobecoin-classic`. I wanted to make sure the general population
