...
###   jobecoincore.org and jobecoin.org and compares them.
...
WORKINGDIR="/tmp/jobecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://jobecoincore.org"
HOST2="https://jobecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="jobecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'jobecoin-',
...
if [ $? -ne 0 ]; then
   echo "jobecoin.org failed to provide signature file, but jobecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "jobecoin.org and jobecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Jobecoin Core binary release signing key installed?"
   fi
